...
int gfc_perform(gfcrequest_t *gfr){
	...
	while(1)
	{
		ssize_t nb = recv(hSocket,buffer,BUFSIZE,0);
	...
	if(close(hSocket) == -1) printf("socket close failed\n");
...
char* stringFromError(gfstatus_t stat)
{
	char *strings[] = {"OK","FILE_NOT_FOUND","ERROR"};
